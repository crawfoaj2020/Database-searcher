...
(define (seperate-rows str)
  (let ([start-time (real-time)]
    ...
    (let* ([rows-marked (pregexp-replace* start-row-pattern str start-row-replace)]
           [end-pass-one (real-time)]
           [search (pregexp-match-all pattern rows-marked)])
      (display (- end-pass-one start-time))
      (display " seperate-rows mark rows\n")
      (display (- (real-time) end-pass-one))
      (display " seperate-rows process row\n")
      search)))
...
(define (read-file-to-string file-path)
  (let ([start-time (real-time)]
        [tx (make-transcoder (latin-1-codec))]
...
(define (set-up-conversion folder db)
  ...
  (execute-sql db "PRAGMA Journel_Mode = OFF")
...
(define (test-convert)
  (let (;[folder "C:\\Users\\AJCRAWFORD\\Documents\\Example from Praveena\\R0273091_2018.06.21_1236\\Logs\\Biomek Logs\\Logs"]
        ...
        [folder "C:\\Users\\AJCRAWFORD\\Documents\\Python\\testFiles\\OneFile"]
        [dest "C:\\Users\\AJCRAWFORD\\Documents\\Python\\testFiles\\Results.db3"])
    (delete-file dest)
